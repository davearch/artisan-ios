...
- (id)init
{
    ...
    if (self) {
        [self.view setBackgroundColor:[UIColor colorWithPatternImage:
                                       [UIImage imageNamed:@"cardboard_flat"]]];
        
        self.searchView = [[SearchView alloc]
        ...
        self.tableView = [[UITableView alloc]
                          initWithFrame:CGRectMake(0,
                                                   self.searchView.bottom,
                                                   self.view.width,
                                                   self.view.height -
                                                   self.searchView.height)];
...
- (void)receivedArtistInformation:(NSNotification *) notification
{
    [UIApplication cancelPreviousPerformRequestsWithTarget:self
                                                  selector:@selector(reallyReloadArtistInformation:)
                                                    object:nil];
    [self performSelector:@selector(reallyReloadArtistInformation:)
...
{
    [UIApplication cancelPreviousPerformRequestsWithTarget:self
                                                  selector:@selector(reallyReloadMixInformation::)
                                                    object:nil];
    [self performSelector:@selector(reallyReloadMixInformation:)
               withObject:notification
               afterDelay:0.3]; // Try to avoid double notifications
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (section == kResultsSectionArtistInformation) {
        return
            receivedArtistInformationResults &&
            (([self.artist.biography length] > 0) || self.artist.imageURL)
        ? 1 : 0;
    }
    // kResultsSectionArtistInformation
    else if (section == kResultsSectionArtistMixes) {
        return
            (receivedArtistMixResults &&
             ([self.artist.mixes count] > 0))
        ? 1 : 0;
    }
    return 0;
}
...
viewForHeaderInSection:(NSInteger)section
{
    ...
    switch (section) {
        case kResultsSectionArtistInformation:
            ...
            artistInformationLabel.textColor = [UIColor colorWithWhite:0.96
                                                                 alpha:1.0];
            ...
            artistInformationLabel.center = CGPointMake(sectionView.width / 2.0,
                                                        sectionView.height / 2.0);
            [sectionView addSubview:artistInformationLabel];
            break;
        case kResultsSectionArtistMixes:
            mixLogo = [[UIImageView alloc]
                       initWithImage:[UIImage imageNamed:@"8TracksLogo"]];
            mixLogo.center = CGPointMake(sectionView.width / 2.0,
                                         sectionView.height / 2.0);
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == kResultsSectionArtistInformation) {
        ...
        ArtistInformationView *artistInformationView =
                [[ArtistInformationView alloc] initWithArtist:self.artist];
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        MixScrollView *mixScrollView =
                [[MixScrollView alloc] initWithMixes:self.artist.mixes];
